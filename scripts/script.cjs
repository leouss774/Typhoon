...
async function geocodeAddress(q) {
  console.log('\n📍 Geocoding…');
  // Try BAN API first (longer timeout + retries)
  const d = await fetchJson(`${BAN_BASE}/search/?q=${encodeURIComponent(q)}&limit=1`,10000,2);
  ...
  // Fallback: try IGN geocoding (may work when BAN fails)
  console.log('  ↳ BAN failed, trying IGN geocoding…');
  const ig = await fetchJson(`${IGN_GEOCODAGE}/search?q=${encodeURIComponent(q)}&limit=1`,8000,1);
  if (ig?.features?.length) {
    const f = ig.features[0], c = f.geometry?.coordinates;
    if (c) {
      const props = f.properties || {};
      // Build a mock banId from IGN data (city code + parcel section)
      const ignCity = props.citycode || '';
      const mockBanId = ignCity ? `${ignCity}000XXXX` : null;
      const r = { lon:c[0], lat:c[1], label:props.label||q, banId:mockBanId };
      console.log(`  ✓ (IGN) ${r.label}\n    ${r.lat.toFixed(5)}, ${r.lon.toFixed(5)}`);
      return r;
    }
  }
  console.error('  ✗ Geocoding failed (BAN + IGN)');
  return null;
}
...
async function fetchBdnbBuilding(banId, communeCode) {
  console.log('\n🏢  BDNB building…');
  if (!banId && !communeCode) { console.log('  ⚠  No banId or commune code'); return null; }
  ...
  // Strategy 3: fallback by commune code (from IGN geocoding or géorisques)
  if (!records?.length && communeCode && communeCode.length >= 5) {
    console.log(`  ↳ Fallback by commune code: ${communeCode}`);
    const f = await fetchJson(`${BDNB_BASE}/donnees/batiment_groupe_complet?code_commune_insee=eq.${communeCode}&limit=5`,8000,1);
    if (f) records = Array.isArray(f)?f:f?.features||[];
  }
...
async function main() {
  ...
  if (hasForm) {
    ...
    // If form was auto-found (not --form= flag) and address differs, clear cached data to avoid contamination
    const formAddr = merged.user_data?.adresse || merged.step7?.address?.adresse || '';
    const addrDiffers = formAddr && ADDRESS !== '8 Allée du Port Maillard 44000 Nantes' && !formAddr.includes(ADDRESS.slice(0,15));
    if (addrDiffers && !parseArg('form')) {
      console.log(`\n⚠️  Adresse différente du formulaire (${formAddr}), effacement des données cachées…`);
      merged.user_data = {};
      merged.step7 = {};
      merged.agent_data.bdnb = { geometry: null };
      merged.agent_data.scores = {};
    } else {
  ...
  if (SKIP_APIS) {
    ...
    [altitude, , building, catnat] = await Promise.all([
      fetchAltitude(geo.lon, geo.lat), fetchCadastralParcel(geo.lon, geo.lat), fetchBdnbBuilding(geo.banId, cc || null), fetchCatnat(cc) ]);
    ...
    // Overwrite form address with geocoded address (fix: shows correct city, not Nantes)
    merged.user_data = merged.user_data || {};
    merged.user_data.adresse = geo.label;
    if (merged.step7?.address) merged.step7.address.adresse = geo.label;
    console.log('\n✅ APIs OK');
  }
